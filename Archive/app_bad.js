...
function prepBarChart(data){
    ...
    var rolledUpFlightsByCarrier = d3.rollup(
        data,
...
function ready(data){
    ...
    {
        yScale.domain(data.map(d => d.yval))

        yAxisDraw.call(yAxis);
        xAxisDraw.call(xAxis);
    }

    let metric = 'carrier_name'

    function click() {
        metric = this.dataset.name;

        const updatedData = d3.rollup(
            data,
            v => (d3.sum(v, leaf => leaf.count_ontime)/d3.sum(v, leaf => leaf.arr_flights)),
            d => d[metric]
        );

        const dataArray = Array.from(updatedData, d => ({ yval: d[0], percentOnTime: d[1] }));
        console.log('---------------------------------')
        console.log(dataArray)
        return dataArray;

        updateyscale(dataArray);
    ...
    //Draw svg  dimensions - draw chart base
